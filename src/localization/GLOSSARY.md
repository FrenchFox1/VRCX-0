...
| Basic Meshes                   | 基本网格               | 一般網格數            | 基本メッシュの数                     |
| Bio                            | 简介                   | 自我介紹              | 自己紹介                             |
| Block                          | 屏蔽                   | 封鎖                  | ブロック                             |
| Blocked Users                  | 已被屏蔽的玩家         | 已封鎖的用戶          | ブロック中のユーザー                 |
| Bones                          | 骨骼                   | 骨頭數                | ボーンの数                           |
| Boop                           | 戳一戳                 | 戳一下                | つっつく                             |
| Bounds                         | 边界范围               | 邊界                  | 範囲 (Bounds)                        |
| Calibrate                      | 校准                   | 校正                  | キャリブレーション                   |
| Camera                         | 相机                   | 相機                  | カメラ                               |
| Capture                        | 相机录制               | 影像擷取              | キャプチャ                           |
| Chatbox                        | 聊天气泡               | 對話框                | チャットボックス                     |
| Clone                          | 克隆                   | 複製                  | クローン                             |
| Cloth Max Vertices             | 布料顶点数             | 布料最大頂點數        | Cloth の最大頂点の数                 |
| Cloth Meshes                   | 布料                   | 布料網格數            | Cloth メッシュの数                   |
| Collider Count                 | 碰撞体                 | 碰撞體數              | コライダーの数                       |
| Color Filter                   | 色彩滤镜               | 色彩濾鏡              | カラーフィルター                     |
| Community Labs                 | 社区实验室             | 社群實驗室            | コミュニティラボ                     |
| Constraint Depth               | 约束链长度             | 約束深度              | コンストレイントの深さ               |
| Constraints                    | 约束                   | 約束數                | コンストレイントの数                 |
| Contact Count                  | 触发器                 | 接觸偵測器數          | コンタクトの数                       |
...
| Download Size                  | 下载文件大小           | 下載容量              | ダウンロードサイズ                   |
...
| Material Slots                 | 材质槽                 | 材質欄位數            | マテリアルスロットの数               |
| Max Avatar Download Size       | 限制模型的最大加载上限 | 角色下載容量上限      | アバターの最大ダウンロードサイズ     |
| Menu                           | 菜单键                 | 選單                  | メニュー                             |
| Mesh Particle Triangles        | 网格粒子三角面总数     | 網格粒子三角面數      | メッシュパーティクルの三角ポリゴン数 |
| Message                        | 消息                   | 訊息                  | メッセージ                           |
| Mic                            | 麦克风                 | 麥克風                | マイク                               |
| Mirror                         | 镜子                   | 鏡子                  | ミラー                               |
| Mode                           | 模式                   | 模式                  | モード                               |
...
| Particle Collisions            | 粒子碰撞               | 粒子碰撞              | パーティクルの衝突                   |
| Particle Systems               | 粒子组件               | 粒子系統數{ln}        | パーティクルシステムの数             |
| Particle Trails                | 粒子尾迹               | 粒子軌跡              | パーティクルトレイル                 |
| Performance Breakdown          | 性能详情               | 效能分析              | パフォーマンスの内訳                 |
| Photo                          | 图片                   | 照片                  | 写真                                 |
| PhysBone Colliders             | PhysBone 碰撞体        | PhysBone 碰撞體數     | PhysBone コライダーの数              |
| PhysBone Collision Check Count | PhysBone 碰撞检测      | PhysBone 碰撞偵測數   | PhysBone 衝突チェックの数            |
| PhysBone Components            | PhysBone 组件          | PhysBone 元件數       | PhysBone コンポーネントの数          |
| PhysBone Transforms            | PhysBone 变换          | PhysBone 影響骨頭數   | PhysBone トランスフォームの数        |
...
| Size                           | 大小                   | 大小                  | サイズ                               |
| Skinned Meshes                 | 蒙皮网格               | 蒙皮網格數            | スキンドメッシュの数                 |
...
| System                         | 系统                   | 系統                  | システム                             |
| Texture Memory                 | 纹理显存占用           | 貼圖容量              | テクスチャメモリー                   |
| Theme                          | 主题                   | 主題                  | テーマ                               |
| Total Max Particles            | 最大粒子总数           | 最大粒子總數          | 最大パーティクルの数                 |
| Trail Renderers                | 尾迹渲染器             | 拖尾渲染器數          | トレイルレンダラーの数               |
...
| Uncompressed Size              | 解压大小               | 未壓縮容量            | 非圧縮サイズ                         |
...
| Blend Shapes                       | 混合形状                   | 混合形狀數            | ブレンドシェイプの数                 |
...
| Leave                              | 退出                       | 退出                  | 退出                                 |
| Line Renderers                     | 线渲染器                   | 線條渲染器數          | ラインレンダラーの数                 |
