...
- **按帳號隔離的本機記錄** — 遊戲記錄等資料按帳號分開儲存，多帳號不再混在同一條時間軸
- **備份與還原** — 一鍵壓縮備份資料庫記錄，支援定期自動備份和多版本保留，隨時還原
- **分享世界收藏集** — 把收藏的世界做成可分享的頁面，對方可以瀏覽、開啟或匯入；也支援單獨分享世界和角色連結
- **社交自動化** — 依時間、實例類型或在場玩家自動切換狀態與簽名；自動接受邀請請求；規則失效後自動還原原有狀態
- **輕量 VR 腕部懸浮層** — 支援 OpenVR（SteamVR）和 **Linux OpenXR（WiVRn 實測通過）**
- **社群主題** — 瀏覽並安裝主題目錄中的主題，設定自訂背景圖片，還可疊加自己的 CSS
- **通知系統** — 桌面通知、語音播報、VR 懸浮層、腕部懸浮層、Webhook 五個通道，按事件類型獨立設定；Webhook 支援 Discord 格式
- 全介面支援完整鍵盤導航
- 面向第三方應用的整合 API — 提供遊戲時房間即時資料，後續逐步擴充
