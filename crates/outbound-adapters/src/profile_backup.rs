...
use vrcx_0_application::profile::{
    OperationGuard, ProfileBackupActionOutcome, ProfileBackupError, ProfileBackupErrorCode,
    ProfileBackupKind, ProfileBackupOutcome, ProfileBackupPhase, ProfileBackupPort,
    ProfileBackupSettings, ProfileBackupState, ProfileBackupStatus, ProfileOperationGate,
...
#[derive(Clone)]
pub struct LocalProfileBackupPort {
    inner: Arc<LocalProfileBackupInner>,
}

pub struct LocalProfileBackupDeps {
...
struct LocalProfileBackupInner {
    app_data: PathBuf,
    control_dir: PathBuf,
    db: Arc<DatabaseService>,
    storage: Arc<StorageService>,
    ...
    state: Mutex<LocalProfileBackupState>,
}

#[derive(Default)]
struct LocalProfileBackupState {
...
impl LocalProfileBackupPort {
    pub fn new(deps: LocalProfileBackupDeps) -> Self {
        let LocalProfileBackupDeps {
            app_data,
            control_dir,
            db,
            storage,
        ...
        Self {
            inner: Arc::new(LocalProfileBackupInner {
                app_data,
                control_dir,
                db,
                storage,
                ...
                state: Mutex::new(LocalProfileBackupState::default()),
            }),
        }
    }
    ...
    fn update_status(&self, update: impl FnOnce(&mut LocalProfileBackupState)) {
        let snapshot = match self.inner.state.lock() {
            Ok(mut state) => {
                update(&mut state);
                state.status.revision = state.status.revision.saturating_add(1);
...
impl ProfileBackupPort for LocalProfileBackupPort {
    fn current_status(&self) -> ProfileBackupStatus {
        LocalProfileBackupPort::current_status(self)
    }

    fn operation_gate(&self) -> ProfileOperationGate {
        LocalProfileBackupPort::operation_gate(self)
    }

    fn discard_pending(&self) -> ProfileBackupActionOutcome {
        LocalProfileBackupPort::discard_pending(self)
    }

    fn dismiss_error(&self) -> ProfileBackupStatus {
        LocalProfileBackupPort::dismiss_error(self)
    }

    fn run_manual(&self, target_path: PathBuf) -> ProfileBackupActionOutcome {
        LocalProfileBackupPort::run_manual(self, target_path)
    }

    fn retry_delivery(&self) -> ProfileBackupActionOutcome {
        LocalProfileBackupPort::retry_delivery(self)
    }

    fn validate_restore(&self, source: &std::path::Path) -> ProfileRestoreValidationOutcome {
        LocalProfileBackupPort::validate_restore(self, source)
    }

    fn request_restore(&self, expected_sha256: &str) -> ProfileRestoreValidationOutcome {
        LocalProfileBackupPort::request_restore(self, expected_sha256)
    }

    fn discard_staged_restore(&self) -> vrcx_0_application_core::Result<()> {
        LocalProfileBackupPort::discard_staged_restore(self)
    }

    fn take_last_restore_result(
        &self,
    ) -> vrcx_0_application_core::Result<Option<ProfileRestoreResult>> {
        LocalProfileBackupPort::take_last_restore_result(self)
    }

    fn cleanup_startup_artifacts(&self) -> vrcx_0_application_core::Result<()> {
        LocalProfileBackupPort::cleanup_startup_artifacts(self)
    }

    fn restore_rollback_state(
        &self,
    ) -> vrcx_0_application_core::Result<ProfileRestoreRollbackState> {
        LocalProfileBackupPort::restore_rollback_state(self)
    }

    fn clear_restore_rollback(&self) -> ProfileRestoreRollbackCleanupOutcome {
        LocalProfileBackupPort::clear_restore_rollback(self)
    }

    fn settings(&self) -> ProfileBackupSettings {
        LocalProfileBackupPort::settings(self)
    }

    fn target_dir_requiring_grant(&self, requested: &ProfileBackupSettings) -> Option<String> {
        LocalProfileBackupPort::target_dir_requiring_grant(self, requested)
    }

    fn set_settings(&self, settings: ProfileBackupSettings) -> ProfileBackupSettings {
        LocalProfileBackupPort::set_settings(self, settings)
    }

    fn start_scheduler(&self) {
        LocalProfileBackupPort::start_scheduler(self);
    }
}
